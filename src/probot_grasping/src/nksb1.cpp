// https://blog.csdn.net/qq_37269094/article/details/82859241
...
int main(int argc, char** argv) {
	Mat src = imread("/home/student/catkin_ws/src/probot_grasping/include/image.png");
	if (src.empty()) 
	{
	...
	findContours(dst, contours, hireachy, CV_RETR_EXTERNAL, CHAIN_APPROX_SIMPLE, Point());
	...
	for (size_t t = 0; t < contours.size(); t++) {
		...
		if (rect.width > width / 2 && rect.width < width - 5) {
			drawContours(drawImage, contours, static_cast<int>(t), Scalar(255, 0, 0), 2, 8, hireachy, 0, Point());
		}
	}
